...
class HardwareDetector:
    ...
    @staticmethod
    def get_current_device_info() -> str | None:
        """Get information about the current default audio device."""
        try:
            result = subprocess.run(
                ["wpctl", "status"], capture_output=True, text=True, check=True, timeout=5
            )
